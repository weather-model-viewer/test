...
void directoryTab(int x, int y, int w, int h, int direc, String txt) {
  ...
  if(mouseBetween(x, y, x+w, y+h) && hmouseBetween(x, y, x+w, y+h)) {
    dir = direc;
    scene = "view";
  }
}
...
void draw() {
  ...
  if(scene == "directory") {
    ...
    fill(0);
    textSize(30);
    text("Directories", width/2-textWidth("Directories")/2, 50);
    for(int i = 0; i < numDirs; i++) {
      directoryTab(40+(i*190)%760, 100+floor(i/4)*130, 150, 90, i, "dir "+str(i));
    }
  }
}
